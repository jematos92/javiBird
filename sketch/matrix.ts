class Matrix {
    ...
    static multiply(a : Matrix, b : Matrix): Matrix { // Matrix product
        if(a.cols !== b.rows) {
        ...
        return new Matrix(a.rows, b.cols).map(
            (e, i, j) => { // Dot product of values in col
                let sum = 0;
    ...
    map(func : (val : number, i : number, j : number) => number): Matrix { // Apply a function to every element of matrix
        for(let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
    ...
    static map(matrix : Matrix, func : (val : number, i : number, j : number) => number): Matrix { // Apply a function to every element of matrix
        return new Matrix(matrix.rows, matrix.cols).map(
            (e, i, j) => func(matrix.data[i][j], i, j)
        );
    }
