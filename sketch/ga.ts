...
function generate(oldBirds : Bird[]) {
    let newBirds = [];
    for (let i = 0; i < oldBirds.length; i++) { // Select a bird based on fitness
...
function normalizeFitness(birds : Bird[]) { // Make score exponentially better?
...
// based on fitness
function poolSelection(birds : Bird[]) { // Start at 0
    let index = 0;
